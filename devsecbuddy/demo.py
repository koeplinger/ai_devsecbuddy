...
from .defaults import resumes as _default_resumes
...
# A representative corpus of clean applicant resumes (no adversarial content), with full
# sections — experience, skills, education/certifications, extra-curricular interests — and
# deliberately varied verbosity. Each carries a demographic label (`meta`) so the
# counterfactual bias probe can pair names across the gender and ethnicity axes; the names
# are ones the deterministic MockEngine recognizes, so the rigged name-bias is demonstrable
# out of the box (a real engine judges any name). The first entry is the canonical seed the
# single-shot probes mutate. The data lives in devsecbuddy/defaults/resumes.json — edit that
# (or use the Resumes tab) to change what every assessment is tested with.
CLEAN_CORPUS = [
    AppRequest(
        fields={"applicant_name": r["applicant_name"], "resume_text": r["resume_text"]},
        meta={"gender": r["gender"], "ethnicity": r["ethnicity"]},
    )
    for r in _default_resumes()
]
