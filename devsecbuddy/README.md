...
(**MockEngine**, **AnthropicEngine**, **VertexEngine**, **GeminiProxyEngine**) and target-pluggable via
...
Bible in [`../docs/`](../docs/). `AnthropicEngine`, `VertexEngine`, and `GeminiProxyEngine`
are fully implemented (their `complete()` methods call the real SDKs / HTTP gateway);
they raise `EngineNotConfigured` until the SDK or credentials are present (roadmap **M6**
provisions the accounts). The backend (M2) and frontend
...
| `engines/` | `AIEngine` protocol + `get_engine`; `mock.py` (the default, rigged, deterministic); `cloud.py` (the `AnthropicEngine`, `VertexEngine`, and `GeminiProxyEngine` cloud engines). |
...
| `prober.py` | Phase 2 — `AdversarialProber` (the five `success_criteria` evaluators, incl. bias counterfactuals). |
| `ledger.py` | Phase 3 — `Ledger` (6-table SQLite schema, findings, dedup, query). |
...
python -m pytest -q               # 124 tests, incl. the tiles.md divergence table
