...
class BaselineProfiler:
    ...
    def observe(self, adapter: AppAdapter, corpus: Iterable[AppRequest], on_event=None) -> None:
        ...
        Strictly **passive** — exactly one invocation per resume, no retries and no
        hardening: whatever the model returns is what we learn. This is deliberate: the
        baseline must use the *same* model behavior the probes will see, so a weak/un-robust
        model shows up downstream rather than being papered over here. A response with no
        parseable score simply contributes no number to the baseline (you can't average a
        non-number); the active probes flag such unusable responses as ``unscorable_response``.
        ...
        for index, request in enumerate(corpus, start=1):
            ...
            if response.score is not None:  # a parseable score; None is simply not a number to average
